...
class ConceptForm extends Component {
  constructor(props) {
    super(props);
    console.log('conceptForm', props.navigation.state.params.person.uid);
    this.state = {
      image: null,
      images: [],
      collection:[],
  ...
  openPicker(){
    ...
    let rand = min + Math.random() * (max - min);
      ...
      let uid = this.props.navigation.state.params.person.username;
    ...
    componentDidMount() {
      this.props.conceptUpdate({prop: 'author_concept', value: this.props.navigation.state.params.person.username});
    ...
    render() {
      ...
      const dpr = this.state.dp ? (
        <TouchableOpacity onPress={() => this.openPicker()}>
          <Image
           ...
           <View>
              <Image style={{width, height: 200, backgroundColor: '#dcdfe5'}} />
      ...
      const dps = this.state.loading ? <ActivityIndicator animating={this.state.loading} /> : (
        ...
        <View style={{flexDirection: "row"}}>
          { dpr }
            ...
            <ScrollView >
              <CardSection>
                ...
                <Input
                label="Author"
                value={this.props.navigation.state.params.person.username}
                edit= {false}
                />
                </CardSection>
                <CardSection >
          ...
          export default connect(mapStateToProps, { conceptUpdate })(ConceptForm);
